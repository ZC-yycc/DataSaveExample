...
public static class SaveManager
{
    ...
    static SaveManager()
    {
        save_file_path_ = Path.Combine(Application.persistentDataPath, "game_save.json");
        Application.quitting += OnApplicationQuitting;

        InitSerializerSettings();
        LoadOrInitSaveFileWithVersionCheck();
    }
    ...
    /// 初始化或加载存档文件
    /// </summary>
    private static void LoadOrInitSaveFileWithVersionCheck()
    {
        // 检查存档文件是否存在
